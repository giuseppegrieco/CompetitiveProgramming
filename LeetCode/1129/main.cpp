#include "solution.cpp"
#include <iostream>

using namespace std;

Solution sol;

string formatList(vector<int> v)
{
  string ans;
  for (int i = 0; i < v.size(); i++)
  {
    ans += to_string(v[i]) + ", ";
  }
  return ans.substr(0, ans.size() - 2);
}

void testCase(int id,
              int n,
              vector<vector<int>> redEdges,
              vector<vector<int>> blueEdges,
              vector<int> expectedResult)
{
  cout << "Test case " << id << endl;
  cout << "Expected: " << formatList(expectedResult) << endl;
  cout << "Result: " << formatList(sol.shortestAlternatingPaths(n, redEdges, blueEdges)) << endl;
}

int main()
{
  testCase(1, 3, {{0, 1}, {1, 2}}, {}, {0, 1, -1});
  testCase(2, 3, {{0, 1}}, {{2, 1}}, {0, 1, -1});
  testCase(3, 3, {{0, 1}, {0, 2}}, {{1, 0}}, {0, 1, 1});
  testCase(4,
           5,
           {{0, 1}, {1, 2}, {2, 3}, {3, 4}},
           {{1, 2}, {2, 3}, {3, 1}},
           {0, 1, 2, 3, 7});

  return 0;
}
